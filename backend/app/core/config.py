...
class Settings(BaseSettings):
    # No default on purpose: the schema uses Postgres-specific types (native
    # UUID columns, native enums), so a silent SQLite fallback here would
    # boot the app against a database it can't actually run correctly on
